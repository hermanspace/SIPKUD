<div class="space-y-6">
    @php
        $user = auth()->user();
        $role = $user->isSuperAdmin() ? 'super_admin' : ($user->isAdminKecamatan() ? 'admin_kecamatan' : ($user->isExecutiveView() ? 'executive_view' : 'admin_desa'));
        $roleLabel = [
            'super_admin' => 'Super Admin (PMD Kabupaten)',
        ...
        $modul = [
            'Master Data' => [
                ...
                'items' => [
                    ['Kelompok & Anggota', 'Daftarkan kelompok usaha lalu anggotanya (NIK wajib). Anggota adalah subjek pinjaman.', ['admin_desa', 'admin_kecamatan', 'super_admin']],
                    ['Akun / COA (Global)', 'Bagan akun berlaku untuk seluruh desa. Hanya Admin Kecamatan & Super Admin yang boleh menambah/mengubah - Admin Desa memakai akun yang tersedia.', ['admin_kecamatan', 'super_admin']],
                    ['Unit Usaha', 'Pisahkan pembukuan per unit (USP, UED-SP, dll). Semua transaksi & laporan bisa difilter per unit.', ['admin_desa', 'super_admin']],
                    ['Sektor Usaha', 'Kategori usaha peminjam - dipakai statistik dashboard.', ['admin_desa', 'admin_kecamatan', 'super_admin']],
                    ['Aset Tetap (BARU)', 'Daftarkan aset (harga, umur ekonomis, akun terkait). Penyusutan garis lurus dijurnal OTOMATIS tiap awal bulan; nilai buku tampil di daftar.', ['admin_desa', 'super_admin']],
                ],
            ],
            'Transaksi Harian' => [
            ...
            'Periode & Tutup Buku' => [
                ...
                'items' => [
                    ['Tutup Periode Bulanan', 'Kunci bulan yang sudah final di menu Manajemen Periode - transaksi terkunci tidak bisa diubah/dihapus. Saldo akhir otomatis jadi saldo awal bulan berikutnya.', ['admin_desa', 'admin_kecamatan', 'super_admin']],
            ...
            'Laporan' => [
                ...
                'items' => [
                    ['Neraca Saldo, Laba Rugi, Neraca', 'Laporan inti bulanan - semuanya otomatis dari buku besar (ledger), tidak bisa diedit manual.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['Arus Kas (BARU)', 'Metode langsung: aktivitas operasi / investasi / pendanaan, per bulan atau setahun penuh.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['Perubahan Ekuitas (BARU)', 'Modal awal + laba berjalan - prive = modal akhir.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['Kolektibilitas & NPL (BARU)', 'Kualitas portofolio pinjaman per kategori umur tunggakan + rasio NPL. Alat pembinaan utama kecamatan/kabupaten.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['CALK (BARU)', 'Catatan atas Laporan Keuangan: kebijakan akuntansi, rincian akun material, kualitas piutang - dibangkitkan otomatis.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['Laporan Tahunan (BARU)', 'Satu PDF berisi 5 komponen wajib PP 11/2021 + kop & tanda tangan resmi - siap untuk Musyawarah Desa.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                    ['LPP UED, Buku Kas, Laporan Akhir USP', 'Laporan manajemen program UED-SP. Laporan Akhir USP kini menghitung SHU = laba bersih + tabel alokasi sesuai AD/ART.', ['admin_desa', 'admin_kecamatan', 'super_admin', 'executive_view']],
                ],
            ],
            'Administrasi Sistem' => [
                ...
                'items' => [
                    ['Pengguna', 'Super Admin membuat semua akun; Admin Kecamatan membuat Admin Desa di wilayahnya.', ['admin_kecamatan', 'super_admin']],
                    ['Pengumuman & Pengaturan', 'Pengumuman tampil di dashboard semua pengguna. Pengaturan: nama instansi & logo (dipakai kop laporan PDF).', ['super_admin']],
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-700">
                    <tr><td class="px-4 py-2.5 font-medium">Super Admin (PMD)</td><td class="px-4 py-2.5">Seluruh kabupaten</td><td class="px-4 py-2.5">Kelola wilayah, pengguna, COA, pengaturan, backup &amp; restore, semua laporan</td></tr>
